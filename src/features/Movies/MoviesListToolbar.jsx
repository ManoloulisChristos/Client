...
function MovieListToolbar({ totalResults, moviesLoaded }) {
  ...
  let queryString = 'sortBy=Default&sort=0&page=1'; //Default query
  ...
  /////// All state values that are used for requesting data from the API are connected to the URL ////////
  ...
  // Handles the case when the URL changed from another source.
  ...
  const handleSpinIncrement = () => {
    if (spinButtonValue < pageCount && moviesLoaded) {
      setSpinButtonValue((n) => (n = n + 1));
    }
  };

  const handleSpinDecrement = () => {
    if (spinButtonValue > 1 && moviesLoaded) {
      setSpinButtonValue((n) => (n = n - 1));
    }
  };
    ...
    <div
      ...
      <div className='movies-list-toolbar__spin-wrapper'>
        ...
        {/* <div
          ...
          <label htmlFor='movies-list-toolbar-persist-checkbox'>Persist</label>
          <input type='checkbox' id='movies-list-toolbar-persist-checkbox' />
        </div> */}
      </div>
    </div>
  );
